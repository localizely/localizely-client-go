...
func main() {
	...
	file := os.NewFile(1234, "some_file") // *os.File | Uploading file. Supported following formats: `Flutter ARB, Android XML, iOS strings, iOS stringsdict, iOS xcstrings, Angular XLF, Gettext PO, Gettext POT, Java properties, Ruby on Rails yaml, .NET resx, flat json, csv, Excel .xlsx, Excel .xls`
	...
	reviewed := true // bool | If uploading translations, that are added, should be marked as Reviewed. For uploading translations that are only modified it will have effect only if `overwrite` is set to `true`. When omitted, file formats with a translation state (iOS xcstrings) set it per translation from that state (`translated` is reviewed, `needs_review` and `new` are not; in the main language only `needs_review` is not reviewed), other formats import as not reviewed. (optional)
 ...
 **file** | ***os.File** | Uploading file. Supported following formats: &#x60;Flutter ARB, Android XML, iOS strings, iOS stringsdict, iOS xcstrings, Angular XLF, Gettext PO, Gettext POT, Java properties, Ruby on Rails yaml, .NET resx, flat json, csv, Excel .xlsx, Excel .xls&#x60; | 
 ...
 **reviewed** | **bool** | If uploading translations, that are added, should be marked as Reviewed. For uploading translations that are only modified it will have effect only if &#x60;overwrite&#x60; is set to &#x60;true&#x60;. When omitted, file formats with a translation state (iOS xcstrings) set it per translation from that state (&#x60;translated&#x60; is reviewed, &#x60;needs_review&#x60; and &#x60;new&#x60; are not; in the main language only &#x60;needs_review&#x60; is not reviewed), other formats import as not reviewed. | 
